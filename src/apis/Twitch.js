...
module.exports = class TwitchAPI extends APIWrapper {
  ...
  async refreshTokens (endpoint, queryParams) {
    await fetch(`https://id.twitch.tv/oauth2/token?client_id=4uuc9u4iezex5zadzidv3f0lfm7vyo&client_secret=${process.env.TWITCH_CLIENT_SECRET}&grant_type=client_credentials`, {
      method: 'POST'
    }).then(res => res.json())
      .then(data => {
  ...
  request (endpoint, queryParams = {}) {
    ...
    return fetch(API_URL + endpoint + `?${qParams.toString()}`, {
      headers: { 'Client-ID': "4uuc9u4iezex5zadzidv3f0lfm7vyo", 'Authorization': `Bearer ${this.accessToken}` }
    }).then(res => res.json())
      .then(data => {
